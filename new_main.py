...
def eval_gemones_multi(genomes, config, simulation_vars, daily_df, queue):
    ...
    for genome_id, genome in genomes:
        ...
        data_tuple = (daily_df.copy(deep=True), net, genome_id)  # Update this line based on your logic
    ...
    for p in processes:
        p.join()
    '''
    # Retrieve results from the queue and assign fitness to genomes
    while not queue.empty():
        genome_id, total_reward = queue.get()
        rewards_lst.append((genome_id, total_reward))
    ...
    # Process the collected rewards
    if not rewards_lst:
        print('No results received !!!\n\n\n')
    else:
        max_entry = max(rewards_lst, key=lambda x: x[1])
        best_gen_id = max_entry[0]
...
'''
def eval_gemones_multi(genomes, config, simulation_vars, daily_df):
    nets = []
    ge = []
    processes = []
    rewards_lst = []  # To collect rewards
    ...
    if not rewards_lst:
        ...
        # save best genome
        # Get the maximum reward value using a list comprehension
        max_entry = max(genomes_with_rewards, key=lambda x: x[1])
        best_gen_id = max_entry[0]
    ...
    # Update the generation counter
    gen_num = local_simulation_vars['gen_num']
    gen_num.value += 1
    local_simulation_vars['gen_num'] = gen_num
# end eval_gemones_multi()
'''
...
def evaluate_genomes_wrapper(genomes, config):
    queue = multiprocessing.Queue()  # Instantiate a queue
